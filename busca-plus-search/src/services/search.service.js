...
class SearchService {
  ...
  async search(query, page = 1, sourceId = null) {
    try {
      ...
      if (sourceId) {
        searchParams.filter_by = `source_id:${sourceId}`;
      }

      searchParams.sort_by = '_text_match:desc,crawled_at:desc';
  ...
  getImageUrl(path) {
    ...
    return `${this.crawlerExternalUrl}/images/${filename}`;
  }
  ...
  async searchImages(query, page = 1, sourceId = null) {
    try {
      ...
      const searchParams = {
        q: query,
        query_by: 'title, description, image_alts',
        page: parseInt(page),
        per_page: perPage,
      ...
      if (sourceId) {
        searchParams.filter_by = `source_id:${sourceId} && has_images:true`;
      }
  ...
  formatImageHit(hit) {
    ...
    if (Array.isArray(doc.images)) {
      images = doc.images.map((img, i) => {
        const imgPath = img.includes('/') ? img.split('/').pop() : img;
        const thumbPath = doc.image_thumbnails?.[i]?.includes('/') 
          ? doc.image_thumbnails?.[i].split('/').pop() 
          : doc.image_thumbnails?.[i];
        return {
          localPath: img,
          thumbnailPath: thumbPath ? `${this.crawlerExternalUrl}/images/${thumbPath}` : `${this.crawlerExternalUrl}/images/${imgPath}`,
          alt: doc.image_alts?.[i] || doc.title,
          width: 0,
          height: 0,
        };
      });
